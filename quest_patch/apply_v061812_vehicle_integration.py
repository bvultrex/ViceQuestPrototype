#!/usr/bin/env python3
"""ViceQuest v0.6.18.12 vehicle integration pass.

Applied after the hardware-safe 0.6.18.5 replay patches.  This deliberately
does not enable the later GTA2 audio/channel upgrade that was part of the Quest
black-screen bisect.

Fixes:
- bounded positional engine voices for nearby world vehicles
...
def must_replace(text: str, old: str, new: str, label: str, count: int = 1) -> str:
    hits = text.count(old)
    if hits < count:
        raise SystemExit(f"Missing v0.6.18.12 anchor {label}: found {hits}, need {count}")
...
# ---------------------------------------------------------------------------
# 3) World engine audio.  Keep the proven local engine voice, but add a bounded
# positional pool.  Quest gets only five extra looping voices; desktop gets ten.
# This fixes silence while on foot and makes nearby traffic audible from inside
# a car without bringing in the black-screen-suspect 18.8 audio patch.
# ---------------------------------------------------------------------------
audio = must_replace(
    audio,
...
''',
    '''var _cache: Dictionary = {}
var _engine_key: String = ""
var _world_engine_players: Array[AudioStreamPlayer3D] = []
var _oneshot_pool: Array[AudioStreamPlayer3D] = []
''',
    "world engine pool var",
)
audio = must_replace(
    audio,
...
''',
    ...
    var world_engine_count: int = 5 if low_power else 10
    for index in range(world_engine_count):
        var world_voice: AudioStreamPlayer3D = AudioStreamPlayer3D.new()
        world_voice.name = "WorldVehicleEngine_%02d" % index
        world_voice.max_distance = 42.0
        world_voice.unit_size = 6.0
        world_voice.attenuation_filter_cutoff_hz = 10000.0
        world_voice.volume_db = -16.0
        world_voice.set_meta("engine_key", "")
        add_child(world_voice)
        _world_engine_players.append(world_voice)

    wasted_player = AudioStreamPlayer.new()
''',
    "create world engine pool",
)

world_audio = r'''func update_world_vehicles(vehicle_map: Dictionary, listener_position: Vector3, local_vehicle_id: int = 0) -> void:
    if _world_engine_players.is_empty():
        return
    var candidates: Array[Dictionary] = []
    for raw_vehicle_id in vehicle_map.keys():
        var vehicle_id: int = int(raw_vehicle_id)
        if vehicle_id == local_vehicle_id:
            continue
        var vehicle: ViceQuestVehicle = vehicle_map[raw_vehicle_id] as ViceQuestVehicle
        if vehicle == null or vehicle.is_destroyed or not vehicle.stream_active:
            continue
        var engine_running: bool = vehicle.driver_id != 0 or vehicle.ai_controlled or absf(vehicle.current_speed) > 0.20
        if not engine_running:
            continue
        var distance: float = vehicle.global_position.distance_to(listener_position)
        if distance > 42.0:
            continue
        candidates.append({"vehicle": vehicle, "distance": distance})

    candidates.sort_custom(func(a, b): return float(a["distance"]) < float(b["distance"]))
    var active_count: int = mini(candidates.size(), _world_engine_players.size())
    for index in range(_world_engine_players.size()):
        var voice: AudioStreamPlayer3D = _world_engine_players[index]
        if index >= active_count:
            if voice.playing:
                voice.stop()
            voice.set_meta("engine_key", "")
            continue
        var vehicle: ViceQuestVehicle = candidates[index]["vehicle"] as ViceQuestVehicle
        var key: String = String(ENGINE_BY_VARIANT.get(vehicle.variant_id, "engine_standard"))
        var previous_key: String = String(voice.get_meta("engine_key", ""))
        if key != previous_key or voice.stream == null:
            voice.stop()
            voice.stream = _looped_stream(key)
            voice.set_meta("engine_key", key)
        ...
        voice.pitch_scale = 0.72 + speed_ratio * (0.44 if vehicle.variant_id == "tank" else 0.58)
        voice.volume_db = lerpf(-18.0, -9.0, speed_ratio)
        if not voice.playing:
            voice.play()

'''
stop_engine_start, _ = func_span(audio, "stop_engine")
audio = audio[:stop_engine_start] + world_audio + audio[stop_engine_start:]
...
''',
    '''            if audio_manager != null:
                var local_vehicle_id_for_audio: int = int(player_vehicle.get(local_id, 0))
                if local_vehicle_id_for_audio != 0 and vehicles.has(local_vehicle_id_for_audio):
                    audio_manager.update_local_vehicle(vehicles[local_vehicle_id_for_audio])
                else:
                    audio_manager.stop_engine()
                audio_manager.update_world_vehicles(vehicles, target, local_vehicle_id_for_audio)
''',
    "world engine main tick",
)

# ---------------------------------------------------------------------------
# 4) Anti-gummyball contacts.  CharacterBody contacts should lose energy and
...
checks = [
    ...
    ("local_vehicle_id_for_audio", main),
    ("synced_cop.enforcement_role != role", main),
    ("previous_profile_hp", cop),
    ("func update_world_vehicles", audio),
    ("WorldVehicleEngine_", audio),
    ("Near-inelastic GTA-style contact", vehicle),
]
for needle, text in checks:
    if needle not in text:
        raise SystemExit(f"v0.6.18.12 verification failed: {needle}")

main_path.write_text(main, encoding="utf-8")
vehicle_path.write_text(vehicle, encoding="utf-8")
cop_path.write_text(cop, encoding="utf-8")
audio_path.write_text(audio, encoding="utf-8")
print("Applied v0.6.18.12: world engines, Cop-driven law cars, durable Agent HP, anti-bounce crashes.")
